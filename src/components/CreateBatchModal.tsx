...
export const CreateBatchModal: React.FC = () => {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-4">
      <div className="w-full max-w-3xl bg-aviation-950 border border-aviation-800 rounded-3xl p-6 shadow-2xl space-y-6 max-h-[90vh] overflow-y-auto animate-fadeIn">
        {/* Header */}
        <div className="flex items-start justify-between border-b border-aviation-800 pb-4">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-2xl bg-gradient-to-br from-skyline-500 to-indigo-600 flex items-center justify-center text-white shadow-glow-cyan">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          {/* Batch Core Info */}
          <div className="p-5 rounded-2xl bg-aviation-900/60 border border-aviation-800 space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  className="w-full bg-aviation-950 border border-aviation-800 rounded-xl px-3.5 py-2.5 text-xs text-white placeholder-slate-500 focus:border-skyline-500 focus:outline-none"
                />
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  className="w-full bg-aviation-950 border border-aviation-800 rounded-xl px-3.5 py-2.5 text-xs text-white placeholder-slate-500 font-mono focus:border-skyline-500 focus:outline-none"
                />
              </div>
            </div>
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-skyline-500/15 hover:bg-skyline-500/25 text-skyline-300 border border-skyline-500/30 text-xs font-semibold"
              >
            ...
            <div className="space-y-2.5">
              {cadets.map((cadet, idx) => (
                <div
                  key={idx}
                  className="p-3.5 rounded-xl bg-aviation-900/40 border border-aviation-800/80 grid grid-cols-1 sm:grid-cols-12 gap-3 items-center"
                >
                  <div className="sm:col-span-4 space-y-1">
                    <label className="text-[10px] font-mono text-slate-400">Cadet Full Name</label>
                    <input
                      type="text"
                      required
                      ...
                      className="w-full bg-aviation-950 border border-aviation-800 rounded-lg px-2.5 py-1.5 text-xs text-white"
                    />
                  </div>

                  <div className="sm:col-span-3 space-y-1">
                    <label className="text-[10px] font-mono text-slate-400">Student ID Code</label>
                    <input
                      type="text"
                      value={cadet.student_number}
                      onChange={(e) => handleCadetChange(idx, 'student_number', e.target.value)}
                      className="w-full bg-aviation-950 border border-aviation-800 rounded-lg px-2.5 py-1.5 text-xs text-white font-mono"
                    />
                  </div>

                  <div className="sm:col-span-4 space-y-1">
                    <label className="text-[10px] font-mono text-slate-400">Class 1 Medical Expiry</label>
                    <input
                      type="date"
                  ...
                  <div className="sm:col-span-1 flex justify-end pt-3 sm:pt-0">
                    <button
                      ...
                      className="p-1.5 rounded-lg bg-rose-500/10 hover:bg-rose-500/20 text-rose-400 border border-rose-500/30 disabled:opacity-30"
                    >
