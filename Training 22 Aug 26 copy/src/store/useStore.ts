...
export type ATOTab =
  ...
  | 'scheduler'    // Operational Scheduling Engine
...
export const useStore = create<ATOStore>((set, get) => ({
  ...
  validation: {
    isValid: true,
    summary: 'Validation engine ready.',
    fdtl: null,
    checks: [],
  },
  ...
  commitSessionBooking: async () => {
    ...
    if (!validation.isValid) {
      ...
      if (fdtlFail) {
        addToast({
          type: 'error',
          title: 'DGCA FDTL Violation (SQL Code P0001)',
      ...
      if (prereqFail) {
        addToast({
          type: 'error',
          title: 'Batch Progression Gatekeeper Violation',
      ...
      if (lockoutFail) {
        addToast({
          type: 'error',
          title: 'Instructor Refresher Lockout Triggered',
      ...
      addToast({
        type: 'error',
        title: 'Booking Blocked by Compliance Engine',
